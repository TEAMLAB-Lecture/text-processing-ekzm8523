...
def normalize(input_string):
    ...
    * 앞뒤 필요없는 띄어쓰기는 제거해야함
    ...
    tmp_input_string = input_string.lower()

    normalized_string = []

    tmp_input_string = tmp_input_string.lstrip()
    tmp_input_string = tmp_input_string.rstrip()

    size = len(tmp_input_string)
    for i in range(size):
        if(tmp_input_string[i] == ' ' and tmp_input_string[i-1] == ' '):
                continue
        normalized_string.append(tmp_input_string[i])
